...
```{r}
### Get data
y <- robust_scrape('http://fantasy.espn.com/apis/v3/games/flb/seasons/2022/segments/0/leagues/49106?view=mTeam')
...
for(i in 1:20) {
  ...
  x <- robust_scrape(glue("http://fantasy.espn.com/apis/v3/games/flb/seasons/2022/segments/0/leagues/49106?scoringPeriodId={sp_id}&view=mBoxscore"))
...
read_csv('historical_playoff_odds_2022.csv') %>%
  filter(matchup_id != params$matchweek_id) %>%
  bind_rows(sim_results %>% mutate('matchup_id' = params$matchweek_id)) %>%
  write_csv('historical_playoff_odds_2022.csv')
```
...
```{r}
history <- read_csv('historical_playoff_odds_2022.csv')
