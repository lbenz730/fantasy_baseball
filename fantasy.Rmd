...
exp_standings <- 
  team_points %>% 
  filter(matchup_id < (wday(Sys.Date()) == 2 & hour(Sys.time()) < 12) + params$matchup_id) %>% 
  group_by(team) %>% 
  summarise(
...
games_left <- (20 - params$matchup_id + !(wday(Sys.Date()) == 2 & hour(Sys.Date()) < 12))
clinch_wins <-  games_left + sort(df$win, decreasing = T)[5] + 1
ferry_clinch_wins <- games_left + sort(df$win, decreasing = T)[12] + 1
playoff_elim_wins <- sort(df$win, decreasing = T)[4] - games_left - 1
ferry_check <- all(sort(df$win)[1:11] > ferry_clinch_wins)
...
gt1 <-
  ...
  text_transform(
    locations = cells_body(
      columns = c(playoffs),
      rows = grepl('12', team) & ferry_check
    ),
    fn = function(x) {
      emo::ji('check')
    }
...
```{r, include = F}
gtExtras::gtsave_extra(gt1 , 'figures/stats.png', vwidth = 3000, selector = 'table')
...
```{r, include = F}
gtExtras::gtsave_extra(gt_top , 'figures/top_performers.png', vwidth = 3000, selector = 'table')
gtExtras::gtsave_extra(gt_top , glue('figures/top_performers/{params$season}/top_performers/week{params$matchup_id}.png'), vwidth = 3000, selector = 'table')
...
### Best Lineup of the Week
```{r, include = F}
best_lineup(params$season, params$matchup_id)
```

<img src="figures/best_lineup.png" class="center" />

### Season All Stars
```{r, include = F}
make_asg_graphics(params$season)
```

<img src="figures/top_performers/2022/all_stars/asg_2022.png" width="1600px" />

<img src="figures/top_performers/2022/all_stars/asg_2022_count.png" class="center" />
...
trans_log <- get_trans_log(params$season)
df_fa <- 
  trans_log %>% 
  filter(transaction_type == 'Free Agent') %>% 
  filter(w_bat * n_games >= 20 | w_sp * n_games >= 5 | w_rp * n_games >= 5) %>% 
  mutate('ppg_vs_avg' = 
           (ppg - df$batting_ppg[13]) * scale_factors$n_bat * w_bat + 
           (ppg - df$rp_ppg[13]) * scale_factors$n_rp * w_rp + 
           (ppg - df$sp_ppg[13]) * scale_factors$n_sp * w_sp) %>% 
  mutate('total_value' = n_points - (end - start + 1)/7 * 
           (df$batting_ppg[13] * scale_factors$n_bat * w_bat + 
              df$rp_ppg[13] * scale_factors$n_rp * w_rp + 
              df$sp_ppg[13] * scale_factors$n_sp * w_sp)) %>% 
  mutate('player_url' = glue('https://a.espncdn.com/combiner/i?img=/i/headshots/mlb/players/full/{player_id}.png&w=350&h=254')) %>% 
...
gt_fa <- 
  ...
  ### Logos
  text_transform(
    locations = cells_body(columns = contains(c('logo', 'player_url'))),
    fn = function(x) {
      web_image(
        url = x,
        height = 50
  ...
  tab_source_note('Min Games for Inclusion: Batter (20), SP (5), RP (5)') %>% 
...
gtExtras::gtsave_extra(gt_fa , glue('figures/top_performers/{params$season}/best_free_agents.png'), vwidth = 3000, selector = 'table')

```

<img src="figures/top_performers/2022/best_free_agents.png" width="1600px" />
...
upset_p <- 
  ...
  group_by(team_home) %>% 
  filter(days_left == min(days_left)) %>% 
  mutate('p' = pmin(win_prob, 1-win_prob)) %>% 
  pull(p) %>% 
  map_dbl(~{1-.x}) %>% 
  prod()
upset_p <- 1-upset_p
...
if(wday(Sys.Date()) == 2 & hour(Sys.time()) < 12) {
  read_csv(glue('data/stats/{params$season}/daily_stats_{params$season}.csv')) %>% 
    filter(matchup_id <= params$matchup_id) %>%  
...
ggplot(history, aes(x = matchup_id, y = playoffs, fill = team)) +
  ...
  scale_x_continuous(breaks = 0:params$matchup_id) +
  labs(x = 'Week',
       y = 'Playoff Odds',
...
ggsave('figures/ferry_odds.png', height = 9/1.2, width = 16/1.2)
```
...
hot_take_batters <- 
  ...
  mutate('cum_games' = cumsum(played),
         ...
         'roll_ppg' = zoo::rollsum(points, 10, fill = NA, align = 'right')/(0.00000000001 + zoo::rollsum(played, 10, fill = NA, align = 'right'))) %>% 
  ...
  map_dfr(~{
    ...
    if(nrow(.x) > 1) {
      ...
      tibble('player' = .x$player[1],
             'form' = .x$form[1],
             'scoring_period_id' = .x$scoring_period_id,
             'roll_ppg' = .x$roll_ppg)
    }
  }) %>% 
  ungroup()
...
```{r, include = F}
gtExtras::gtsave_extra(gt_trades , 'figures/trades.png', vwidth = 3000, selector = 'table')
