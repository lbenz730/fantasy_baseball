...
for(i in 1:20) {
  ...
  home <- schedule_$home %>% dplyr::slice(1:120) ### Reg Season
  away <- schedule_$away %>% dplyr::slice(1:120)
  
  schedule_ <- select(schedule_,
                      'matchup_id' = matchupPeriodId,
                      'game_id' = id) %>% dplyr::slice(1:120)
...
relief_starts <- 
  ...
  filter(in_lineup) %>% 
  filter(pitcher) %>% 
  group_by(player, team_id, matchup_id) %>% 
...
df_trades <- 
  ...
  filter(matchup_id < params$matchweek_id) %>% 
  bind_rows(get_trades(params$matchweek_id))
...
df_wp <- plot_wp(params$season, params$matchweek_id)
```

```{r}
const <- 0
if((wday(Sys.Date()) == 1 & hour(Sys.time()) <= 20)) {
  const <- 1
}

df_wp %>%
  group_by(team_home) %>% 
  filter(days_left == min(days_left)+const) %>% 
  ungroup() %>% 
  mutate('statement' = paste0(team_home, ' (', round(100 * win_prob), '%) vs. ', team_away, ' (', round(100 * (1-win_prob)), '%)')) %>% 
  pull(statement) %>% 
  paste(collapse = '\n') %>% 
  paste0('Win Probability Week: ', params$matchweek_id, '\n-------------------\n', .) %>% 
  write('wp.txt')

if(wday(Sys.Date()) == 2) {
  read_csv(glue('data/stats/{params$season}/daily_stats_{params$season}.csv')) %>% 
    filter(matchup_id <= params$matchweek_id) %>%  
    bind_rows(get_matchup_stats(params$matchweek_id + 1, season = params$season)) %>% 
    write_csv(glue('data/stats/{params$season}/daily_stats_{params$season}.csv'))
  df_lookahead <- plot_wp(params$season, params$matchweek_id + 1, plot = F)
  ...
  df_lookahead %>%   
    ...
    pull(statement) %>% 
    paste(collapse = '\n') %>% 
    paste0('Pre-Match Lines Week: ', params$matchweek_id+1, '\n-------------------\n', .) %>% 
    write('week_lines.txt')
}
```
...
ggplot(df_points, aes(x = scoring_period_id, y = rank)) + 
  facet_wrap(~team) + 
  geom_vline(data = df_start %>% filter(matchup_id <= params$matchweek_id), aes(xintercept = end_period), lty = 2) + 
...
for(i in 1:nrow(traded_players)) {
  ...
  tmp_before <- 
    ...
    mutate('ppg_vs_avg' = case_when(rp == 0 & sp == 0 ~ ppg - df$batting_ppg[13],
                                    rp > 0 & sp == 0 ~ ppg - df$rp_ppg[13],
                                    sp > 0 ~ ppg - df$sp_ppg[13])) %>% 
...
df_trades <- 
  ...
  select(trade_id_1, 
         ...
         points_after_1, played_after_1, ppg_after_1, ppg_vs_avg_after_1,
         player_2, player_url_2, team_2, logo_2,
         points_before_2, played_before_2, ppg_before_2, ppg_vs_avg_before_2,
         points_after_2, played_after_2, ppg_after_2, ppg_vs_avg_after_2) 
...
gt_trades <- 
  ...
  tab_style(
    ...
    locations = list(
      cells_body(
        columns = contains('ppg_vs_avg_after_1')
      )
    )
  ) %>%
  ...
  cols_label(
    ...
    'ppg_after_1' = 'PPG',
    'ppg_after_2' = 'PPG',
    'ppg_vs_avg_before_1' = 'PPG vs. League Avg.',
    'ppg_vs_avg_before_2' = 'PPG vs. League Avg.',
    'ppg_vs_avg_after_1' = 'PPG vs. League Avg.',
    'ppg_vs_avg_after_2' = 'PPG vs. League Avg.') %>% 
...
```
<img src="figures/trades.png" height="250px" width="1500px" />
