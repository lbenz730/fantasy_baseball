...
df_wp <- plot_wp(params$season, params$matchup_id)
```
...
df_bat_stocks <- 
  ...
  group_split() %>% 
  map_dfr(~{
    tmp <- approx(.x$scoring_period_id, .x$roll_ppg, n = 500)
...
df_pitch_stocks <- 
  ...
  map_dfr(~{
    tmp <- approx(.x$scoring_period_id, .x$roll_ppg, n = 500)
    tmp2 <- approx(.x$scoring_period_id, .x$roll_qs_pct, n = 500)
    tibble('team_id' = .x$team_id[1],
           'scoring_period_id' = tmp$x,
           'roll_ppg' = tmp$y,
           'roll_qs_pct' = tmp2$y)
...
ggplot(df_rp_stocks, aes(x = scoring_period_id, y = roll_ppg)) + 
  facet_wrap(~team) +
  geom_hline(yintercept = df$rp_ppg[13], lty = 2, alpha = 0.3) + 
  geom_line(aes(col = roll_ppg), lwd = 1.2, lineend = 'round') + 
  scale_color_gradient2(low = 'blue', mid = 'grey', high = 'red', midpoint = df$rp_ppg[13]) +
  labs(x = 'Day of Season',
       y = 'RP Points/Game',
       title = 'Relief Pitching PPG 10 Day Rolling Average') 
